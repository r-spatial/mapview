#' Add a RGB RasterStack - RasterBrick image as a layer
#'
#' @description
#' Create a Red-Green-Blue image overlay from a \code{RasterStack} /
#' \code{RasterBrick} object based on three layers.
...
#'     addRasterRGB(plainview::poppendorf, 5,4,3, group = "False colours") %>%
#'     addLayersControl(
...
addRasterRGB <- function(map, x, r = 3, g = 2, b = 1,
  ...
  out <- addRasterImage(map = m, x = xout[[r]], colors = p, ...)
